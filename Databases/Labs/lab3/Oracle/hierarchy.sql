...
update roles set status=3 where roleName like '%Operator%';
...
begin
open cur for select LPAD(' ',3*LEVEL)||roleName, status, LEVEL as node_level  from ROLES start with status=node connect by nocycle prior status=node;
LOOP
FETCH cur
